...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
			...
			$this->envato_username = apply_filters( $this->theme_name . '_theme_setup_wizard_username', 'Whitebox-Studio' );
			$this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', '//whiteboxstud.io/envato/api/server-script.php' );
		...
		public function init_actions() {
            ...
            add_action( 'wp_ajax_wtbx_update_notice_handler', array($this,'ajax_notice_handler') );
		...
		public function admin_menus() {
            ...
            $add_subpage_function( 'scape-dashboard', esc_html__( 'Setup Wizard', 'scape' ), esc_html__( 'Setup Wizard', 'scape' ), 'manage_options', $this->page_slug, array(
                $this,
                'setup_wizard',
            ) );
		}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'scape' ),
		...
		public function setup_wizard() {
			...
			wp_register_script( 'scape-blockui', WTBX_URI . '/library/js/jquery.blockUI.js', array( 'jquery' ), '2.70', true );
			wp_register_script( 'scape-envato-setup', $this->plugin_url . 'js/envato-setup.js', array(
				'jquery',
				'scape-blockui',
			), $this->version );
			wp_register_script( 'scape-dashboard', WTBX_URI . '/library/js/dashboard.js', array( 'jquery' ), SCAPE_VERSION, true );
			wp_localize_script( 'scape-envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'scape' ),
			) );
			...
			wp_enqueue_style( 'scape-envato-setup', $this->plugin_url . 'css/envato-setup.css', array(
				'wp-admin',
				'dashicons',
				'install',
			), $this->version );
			...
			wp_enqueue_style( 'wtbx-theme-dashboard', get_template_directory_uri() . '/library/css/theme-dashboard.css', array(), SCAPE_VERSION, 'all' );
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'scape' ) . '</ti' . 'tle>'; ?>
			<?php wp_print_scripts( 'scape-blockui' ); ?>
			<?php wp_print_scripts( 'scape-envato-setup' ); ?>
            <?php wp_print_scripts( 'scape-dashboard' ); ?>
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard"
			   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'scape' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
					...
					<li>
						<form method="post">
							...
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Reset font style and colors', 'scape' ); ?>" name="save_step"/>
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'scape' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'scape' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'scape' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'scape' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'scape' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'scape' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'scape' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.', 'scape' ),
						'installing'       => esc_html__( 'Installing.', 'scape' ),
			...
			$content['widgets'] = array(
				...
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'scape' ),
				'pending'          => esc_html__( 'Pending.', 'scape' ),
				'installing'       => esc_html__( 'Installing Default Widgets.', 'scape' ),
				'success'          => esc_html__( 'Success.', 'scape' ),
			...
			$content['settings'] = array(
				'title'            => esc_html__( 'Settings', 'scape' ),
				'description'      => esc_html__( 'Configure default settings.', 'scape' ),
				'pending'          => esc_html__( 'Pending.', 'scape' ),
				'installing'       => esc_html__( 'Installing Default Settings.', 'scape' ),
				'success'          => esc_html__( 'Success.', 'scape' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content', 'scape' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'scape' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'scape' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e( 'Item', 'scape' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'scape' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'scape' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'scape' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'scape' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'scape' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'scape' ),
					'logs'    => $this->logs,
					'errors'  => $this->errors,
				) );
			}
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'scape' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'scape' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'scape' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'scape' ), size_format( $max_size ) ) );
				}
			}
		...
		public function envato_setup_color_style() {
			...
			?>
            <h1><?php esc_html_e( 'Site Style', 'scape' ); ?></h1>
            <form method="post">
                <p><?php esc_html_e( 'Please choose your site style below.', 'scape' ); ?></p>
                ...
                <div class="theme-presets">
                    <ul>
	                    ...
	                    foreach ( $this->site_styles as $style_name => $style_data ) {
		                    ?>
                            <li<?php if ($style_name == $current_style) {echo ' class="current" ';} ?>>
                                <a href="#" data-style="<?php echo esc_attr( $style_name ); ?>"><img
                                            src="<?php echo esc_url(get_template_directory_uri() .'/plugins/envato-setup/images/'.$style_name.'/style.jpg');?>"></a>
                            </li>
	                    <?php } ?>
                    </ul>
                </div>
                ...
                <p class="envato-setup-actions step">
                    <input type="submit" class="button-primary button button-large button-next"
                           value="<?php esc_attr_e( 'Continue', 'scape' ); ?>" name="save_step"/>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'scape' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo', 'scape' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'scape' ), '<a href="//studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'scape' ); ?></a>
						</td>
					</tr>
				</table>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'scape' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'scape' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'scape' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
							<em>On the next page you will be asked to Login with your ThemeForest account and grant
								permissions to enable Automatic Updates. If you have any questions please <a
									href="//whitebox.ticksy.com/" target="_blank">contact us</a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'scape' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'scape' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="//whitebox.ticksy.com/" target="_blank">http://whitebox.ticksy.com/</a>
				and includes:</p>
			<ul>
			...
			<ul>
				<li>Customization services (this is available through <a
						href="//studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall"
		...
		public function envato_setup_ready() {
			...
			<a href="//twitter.com/share" class="twitter-share-button"
			   data-url="//themeforest.net/user/dtbaker/portfolio?ref=dtbaker"
			...
			<p>Please come back and <a href="//themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. <br/>Follow <a href="//twitter.com/dtbaker" target="_blank">@dtbaker</a>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					...
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="//twitter.com/dtbaker"
						                             target="_blank"><?php esc_html_e( 'Follow @dtbaker on Twitter', 'scape' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your new website!', 'scape' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					...
					<ul>
						<li class="documentation"><a href="//dtbaker.net/envato/documentation/"
						                             target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'scape' ); ?></a>
						</li>
						<li class="howto"><a href="//wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'scape' ); ?></a>
						</li>
						<li class="rating"><a href="//themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'scape' ); ?></a></li>
						<li class="support"><a href="//dtbaker.net/envato/"
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class         = 'error';
					echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'scape' ), $error_message ) . '</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'scape' ), $this->envato_username ),
				...
				if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            <p><?php
                            esc_html_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.', 'scape' );
                                ?></p>
                            <p>
                            <?php printf( wp_kses_post( __('<a class="button button-primary" href="%s">Activate License</a>', 'scape' ) ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
                            </p>
                        </div>
