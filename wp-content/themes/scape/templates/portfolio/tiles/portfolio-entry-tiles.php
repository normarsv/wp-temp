...
<article id="portfolio-<?php the_ID(); ?>" <?php post_class('portfolio-entry clearfix wtbx-masonry-entry wtbx-grid-entry wtbx-grid-anim-container '.$overlay_trigger); ?>>
	...
	<div class="portfolio-tiles-inner wtbx-entry-inner clearfix wtbx-element-reveal wtbx_appearance_animation wtbx-grid-anim-<?php echo esc_attr($animation_style); ?>">
		<div class="portfolio-tiles-box">
			...
			<?php if ( $click_action === 'link' ) : ?>
				<a class="portfolio-entry-overlay portfolio-tiles-overlay" href="<?php the_permalink(); ?>" rel="bookmark">
					<div class="portfolio-entry-bg portfolio-tiles-bg"><?php echo wtbx_portfolio_overlay_content($overlay_content, $click_action, $item_layout); ?></div>
				</a>
			<?php elseif ( $click_action === 'gallery_all' ) :
				...
				?>
				<a class="portfolio-entry-overlay portfolio-tiles-overlay wtbx-lightbox-item"
					<?php echo (wtbx_option('portfolio-archive-link') === '1' ? ' data-itemlink="'.esc_attr(get_the_permalink()).'"' : '') ?>
					<?php // This variable does not contain unescaped dynamic data - see lines: 74, 80, 83, 86, 96, 100, 104 ?>
                   ...
                   data-thumbimage="<?php echo esc_url(wp_get_attachment_image_url( $imgID, 'medium' )); ?>"
					<?php // These variables have been safely escaped on lines: 56, 63, 116, 118, 120 ?>
			...
			<?php elseif ( $click_action === 'gallery_item' ) :
				...
				if (!empty($attachments)) :
					...
					?>
                    <div class="portfolio-entry-overlay portfolio-tiles-overlay wtbx-lightbox-item"
						<?php echo (wtbx_option('portfolio-archive-link') === '1' ? ' data-itemlink="'.esc_attr(get_the_permalink()).'"' : '') ?>
                         data-dynamic="1"
	                    ...
	                    <?php // These variables have been safely escaped on lines: 56, 63, 165, 167 ?>
                        <?php echo $data_caption_primary_escaped, $data_caption_secondary_escaped, $data_share_escaped; ?>>
                        <div class="portfolio-entry-bg portfolio-tiles-bg"><?php echo wtbx_portfolio_overlay_content($overlay_content, $click_action, $item_layout); ?></div>
                    </div>
				<?php elseif ( $item_layout === 'video' ) :
					...
					<a class="portfolio-entry-overlay portfolio-tiles-overlay wtbx-lightbox-item"
						<?php // This variable does not contain unescaped dynamic data - see lines: 136, 185, 188, 191 ?>
                        <?php echo $type_escaped; ?>
						<?php echo (wtbx_option('portfolio-archive-link') === '1' ? ' data-itemlink="'.esc_attr(get_the_permalink()).'"' : '') ?>
                       data-dynamic="1"
                       data-dynamicel="<?php echo esc_attr($iframe_url); ?>"
						<?php // These variables have been safely escaped on lines: 56, 63, 195, 197, 199 ?>
                        <?php echo $data_caption_primary_escaped, $data_caption_secondary_escaped, $data_share_escaped; ?>>
						<div class="portfolio-entry-bg portfolio-tiles-bg"><?php echo wtbx_portfolio_overlay_content($overlay_content, $click_action, $item_layout); ?></div>
					</a>
				...
				<?php elseif ( $item_layout === 'audio' ) :
					...
					<a class="portfolio-entry-overlay portfolio-tiles-overlay wtbx-lightbox-item"
						<?php echo (wtbx_option('portfolio-archive-link') === '1' ? ' data-itemlink="'.esc_attr(get_the_permalink()).'"' : '') ?>
						<?php // This variable does not contain unescaped dynamic data - see lines: 217, 222, 225, 228 ?>
                        <?php echo $type_escaped; ?>
                       data-dynamic="1"
                       data-title="<?php echo esc_attr($title); ?>"
                       data-poster="<?php echo esc_attr( wp_get_attachment_image_url( get_post_meta($postID, 'portfolio-item-audio-thumbnail_id', 1 ), 'full' )); ?>"
                       data-dynamicel="<?php echo esc_attr($iframe_url); ?>"
						<?php // These variables have been safely escaped on lines: 56, 63, 234, 236, 238 ?>
                        <?php echo $data_caption_primary_escaped, $data_caption_secondary_escaped, $data_share_escaped; ?>>
						<div class="portfolio-entry-bg portfolio-tiles-bg"><?php echo wtbx_portfolio_overlay_content($overlay_content, $click_action, $item_layout); ?></div>
					</a>
			...
			<?php elseif ( $click_action === 'preview' ) : ?>
				<div class="portfolio-entry-overlay portfolio-tiles-overlay wtbx-preview-trigger" data-postid="<?php echo esc_attr($postID); ?>">
					<div class="portfolio-entry-bg portfolio-tiles-bg"><?php echo wtbx_portfolio_overlay_content($overlay_content, $click_action, $item_layout); ?></div>
				</div>
			<?php endif; ?>
		...
		<?php if ( $meta_primary || $meta_secondary ) : ?>
			<div class="portfolio-entry-meta">
                <a href="<?php the_permalink(); ?>" class="portfolio-tiles-button">
                    <i class="scape-ui-chevron-right"></i>
                </a>
				<?php
					if ( $meta_primary ) {
						echo wtbx_portfolio_meta_content($meta_primary, $postID, 'portfolio-meta-primary', strpos($meta_primary, '_link') !== false);
                    }
					if ( $meta_secondary ) {
						echo wtbx_portfolio_meta_content($meta_secondary, $postID, 'portfolio-meta-secondary', strpos($meta_primary, '_link') !== false);
                    }
				?>
			</div>
		<?php endif; ?>
