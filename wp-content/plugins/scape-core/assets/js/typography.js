(function( $ ) {
	...
	function wtbx_match_variants($typography) {
		var object	= $typography.find('.wtbx_typography_hidden').data('fonts');
		...
		for( var i=0; i<variants_array.length; i++ ) {
			...
			$variant_options += '<option value="'+( variants_array[i] === 'inherit' ? '' : variants_array[i] )+'"' + ( variants_array[i] === value['variants'] ? ' selected="selected"' : '' ) + '>'+curr_label+'</option>';
		}
	...
	$(document).ready(function() {
		...
		$('.wtbx_typography_wrapper').each(function() {
			var $this = $(this);
			...
			$this.find('.wtbx_typography_select').on('change blur', function() {
				wtbx_typography_update($this);
			});

			$this.find('.wtbx_typography_input').on('keyup input', function() {
				wtbx_typography_update($this);
			});
