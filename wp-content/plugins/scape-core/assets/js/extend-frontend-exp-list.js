(function($) {
	...
	window.InlineShortcodeView_vc_expandable_list = window.InlineShortcodeViewContainer.extend( {
		events: {},
		childTag: 'vc_expandable_list_item',
		activeClass: 'active',
		...
		initNav: function() {
			var $list_item = this.$el.find('.wtbx_vc_exp_list_item');
			...
			$list_item.each(function() {
				...
				$trigger.unbind().on('click', function(e) {
					...
					$(this).closest('.wtbx_vc_exp_list_item').toggleClass('active');
				});
			});
		},
		...
		buildSortable: function () {
			...
			if ( this.$el ) {
				this.$el.find( '.wtbx_vc_expandable_list_inner' ).sortable( {
					forcePlaceholderSize: true,
					placeholder: 'wtbx_tabs_placeholder wtbx_vc_exp_list_item', // TODO: fix placeholder
					...
					items: '> .vc_element',
					handle: '.vc_child-element-move, .wtbx_exp_list_title_wrapper',// TODO: change vc_column to vc_tta_section
		...
		getPanelsList: function () {
			return this.$el.find( '.wtbx_vc_expandable_list_inner' );
		},
	...
	window.InlineShortcodeView_vc_expandable_list_item = window.InlineShortcodeViewContainerWithParent.extend( {
		events: {
		...
		allowAddControl: function () {
			return vc_user_access().shortcodeAll( 'vc_expandable_list_item' );
		},
		clone: function ( e ) {
		...
		changed: function () {
			if ( this.allowAddControlOnEmpty() && 0 === this.$el.find( '.vc_element[data-tag]' ).length ) {
				this.$el.addClass( 'vc_empty' ).find( '.wtbx_exp_list_content' ).addClass(
					'vc_empty-element' );
			} else {
				this.$el.removeClass( 'vc_empty' ).find( '.wtbx_exp_list_content.vc_empty-element' ).removeClass(
					'vc_empty-element' );
			}
		},
		moveClasses: function () {
			...
			panelClassName = this.$el.find( '.wtbx_vc_child_container' ).get( 0 ).className;
