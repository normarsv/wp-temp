(function($) {
	...
	vc.cloneMethod_vc_content_slide = function ( data, model ) {
	...
	window.InlineShortcodeView_vc_content_slider = window.InlineShortcodeView_vc_tta_accordion.extend( {
		already_build: false,
		active_model_id: false,
		$tabsNav: false,
		active: 0,
		activeClass: 'active',
		childTag: 'vc_content_slide',
		render: function () {
			window.InlineShortcodeView_vc_content_slider.__super__.render.call( this );
		...
		initNav: function() {
			var $nav		= this.$el.find('.wtbx_slider_nav'),
				$content	= this.$el.find('.wtbx_content_slider_inner'),
				$dots		= this.$el.find('.wtbx_dots'),
				$prev		= this.$el.find('.wtbx_arrow_prev'),
				$next		= this.$el.find('.wtbx_arrow_next');

			$nav.find('.wtbx_slider_nav_item').unbind().on('click', function() {
				if ( !$(this).hasClass('active') ) {
					$nav.children().removeClass('active');
					$content.find('.vc_vc_content_slide').removeClass('active');
			...
			$prev.unbind().on('click', function() {
				...
				$content.find('.vc_vc_content_slide').eq(index).addClass('active');
			});
			...
			setTimeout(function() {
				if ( !$nav.find('.active').length || !$content.find('.vc_vc_content_slide.active').length ) {
					$nav.children().removeClass('active');
					$content.find('.vc_vc_content_slide').removeClass('active');
					$dots.find('.dot').removeClass('is-selected');
					$nav.children().eq(0).addClass('active');
					$dots.find('.dot').eq(0).addClass('is-selected');
					$content.find('.vc_vc_content_slide').eq(0).addClass('active');
				}
			});

			this.$el.find('.wtbx_content_slider_wrapper').addClass('wtbx_slider_init');
		},
		createTabs: function () {
		...
		sectionUpdated: function ( model, justAppend ) {
			...
			var $tabEl,
				$navigation,
				sectionId,
				html, title, models, index, tabAdded, $content, $dots;
			tabAdded	= false;
			sectionId	= model.get( 'id' );
			$navigation	= this.$el.find( '.wtbx_slider_nav' );
			$content	= this.$el.find('.wtbx_content_slider_inner');
			$dots		= this.$el.find('.wtbx_dots');
			$tabEl		= $navigation.find( '[data-vc-target="[data-model-id=' + sectionId + ']"]' );
			title		= 'Slide ' + ($navigation.children().length + 1);

			if ( $tabEl.length ) {
				html = '<div class="wtbx_slider_nav_title">' + title + '</div>';
				$tabEl.html( html );
			} else {
				var $element;
				html = '<div class="wtbx_slider_nav_title">' + title + '</div>';
				$element = $( '<li class="wtbx_slider_nav_item" data-vc-target-model-id="' + sectionId + '" data-vc-target="[data-model-id=' + sectionId + ']" data-vc-tab>' + html + '</li>' );
				if ( true !== justAppend ) {
				...
				$content.find('.vc_vc_content_slide').removeClass(this.activeClass);
				$dots.find('.dot').removeClass('is-selected');
			...
			$navigation.find('.wtbx_slider_nav_item').each(function() {
				$(this).html('Slide ' + ($(this).index()+1));
			});
		...
		getNextTab: function ( $viewTab ) {
			...
			$navigationSections = this.$el.find( '.wtbx_slider_nav' ).children();
		...
		removeSection: function ( modelId ) {
			var $viewTab, $nextTab, tabIsActive, $dots;

			$viewTab = this.$el.find( '.wtbx_slider_nav [data-vc-target="[data-model-id=' + modelId + ']"]');
			...
			if ( tabIsActive ) {
				...
				this.$el.find( '.wtbx_slider_nav' ).children().eq($nextTab).trigger( 'click' );
			}

			// Remove tab from navigation
			this.buildPagination();

			this.$el.find( '.wtbx_slider_nav' ).find('.wtbx_slider_nav_item').each(function() {
				$(this).html('Slide ' + ($(this).index()+1));
			});
		},
		buildSortableNavigation: function () {
			...
			this.$el.find( '.wtbx_slider_nav' ).sortable( {
				items: '.wtbx_slider_nav_item',
				forcePlaceholderSize: true,
				forceHelperSize: false,
				placeholder: 'wtbx_tabs_placeholder wtbx_slider_nav_item',
		...
		updateSorting: function ( event, ui ) {
			window.InlineShortcodeView_vc_content_slider.__super__.updateSorting.call( this, event, ui );
			this.updateTabsPositions( this.getPanelsList() );
		},
		updateSortingNavigation: function () {
			var $tabs, self;
			self = this;
			$tabs = this.$el.find( '.wtbx_slider_nav' );
			// we are sorting a tabs navigation
			$tabs.find( '> .wtbx_slider_nav_item' ).each( function () {
	...
	window.InlineShortcodeView_vc_content_slide = window.InlineShortcodeViewContainerWithParent.extend( {
		events: {
		...
		render: function () {
			...
			window.InlineShortcodeView_vc_content_slide.__super__.render.call( this );
			...
			var params = vc.shortcodes.get( this.model.get( 'parent_id' ) ).get( 'params' ),
				slides;
			if ( params.style === 'default' || params.style === 'fixed' || params.style === 'fixed_content' || params.style === 'boxed_simple' ) {
				// slides = params.slides_to_show;
				if ( !this.$el.find('> .wtbx_vc_content_slide:not(.wtbx_slide_placeholder) > .wtbx_slide_placeholder').length ) {
					for( var i=0; i<8; i++ ) {
						this.$el.find('> .wtbx_vc_content_slide').append('<div class="wtbx_slide_placeholder"></div>');
					}
				}
			}
		...
		allowAddControl: function () {
			return vc_user_access().shortcodeAll( 'vc_content_slide' );
		},
		clone: function ( e ) {
			vc.ttaSectionActivateOnClone = true;
			window.InlineShortcodeView_vc_content_slide.__super__.clone.call( this, e );
		},
		addSibling: function ( e ) {
			window.InlineShortcodeView_vc_content_slide.__super__.addSibling.call( this, e );
		},
		parentChanged: function () {
			window.InlineShortcodeView_vc_content_slide.__super__.parentChanged.call( this );
			this.refreshContent( true );
			return this;
		},
		changed: function () {
			if ( this.allowAddControlOnEmpty() && 0 === this.$el.find( '.vc_element[data-tag]' ).length ) {
				this.$el.addClass( 'vc_empty' ).find( '.wtbx_content_slide_inner' ).addClass(
					'vc_empty-element' );
			} else {
				this.$el.removeClass( 'vc_empty' ).find( '.wtbx_content_slide_inner.vc_empty-element' ).removeClass(
					'vc_empty-element' );
			}
		},
		moveClasses: function () {
			...
			// panelClassName = this.$el.find( '.wtbx_vc_content_slide' ).get( 0 ).className;
		...
		destroy: function ( e ) {
			...
			window.InlineShortcodeView_vc_content_slide.__super__.destroy.call( this, e );
