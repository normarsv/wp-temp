<?php
$output = $link = $el_class = $unique_class = $style = $icon = $heading_typography = $opposite_float = $lazy =
$row_align = $bg_image_el = $bg_style = $custom_bg_effect = $max_width = $fadeout = $textalign_small =
$bg_parallax = $bg_scale = $bg_mousemove = $bg_video_el = $bg_video_poster = $video_poster = $video_sound =
$overlay_color = $bg_anim_el = $anim_bg_styles = $animated_layer = $anim_direction = $anim_bg_style = $anim_disable =
$segments_style = $disable_margins = $disable_borders = $disable_padding = $disable_columns_gap = $disable_css_animation =
$row_height = $row_min_height = $css_animation = $css_animation_easing = $css_animation_duration = $css_animation_delay =
$row_show_on = $bg_parallax_cont_class = $bg_scale_elem_class = $bg_parallax_elem_class = $bg_overlay = $row_alignment = $custom_bg_type = '';
...
//$shortcode_class[]  = $row_side_space;
...
$fadeout !== '' ? $shortcode_class[] = 'wtbx-fadeout' : null;
...
$anim_disable !== '' ? $shortcode_class[] = 'wtbx_anim_disable_' . $anim_disable : null;
...
// background overlay
if ( !empty(wtbx_vc_color_styles_bg($overlay_color)) ) {
	$bg_overlay = '<div class="wtbx_row_bg_overlay"></div>';
}
...
$output  = '';
$output .= '<div class="'.esc_attr($css_class).'"'.($row_id !== '' ? ' id="'.esc_attr($row_id).'"' : '').($anchor !== '' ? ' data-anchor="'.esc_attr($anchor).'"' : '').($nav_skin !== '' ? ' data-skin="'.esc_attr($nav_skin).'"' : '').'>';
...
!empty($custom_bg_color) ? $js_styles .= $unique_class_css.' .wtbx_row_bg {' . wtbx_vc_color_styles_bg($custom_bg_color) . '}' : null;
!empty($bg_style) ? $js_styles .= $unique_class_css.' .wtbx-bg-image .wtbx-bg-image-inner .wtbx_row_bg_image {' . $bg_style . '}' : null;
!empty($anim_bg_style) ? $js_styles .= $unique_class_css.' .wtbx_row_bg_anim_inner .wtbx-bg-image .wtbx_anim_image {' . $anim_bg_style . '}' : null;
!empty($overlay_color) ? $js_styles .= $unique_class_css.' .wtbx_row_bg_overlay {' . wtbx_vc_color_styles_bg($overlay_color) . '}' : null;
if (!empty($animated_layer) ) {
	!empty($anim_bg_keyframes) ? $js_styles .= $unique_class_css.' .wtbx_row_bg_anim_inner .wtbx_anim_image { -webkit-animation: '.$unique_class.' '.$anim_time.'s linear infinite; -moz-animation: '.$unique_class.' '.$anim_time.'s linear infinite; -o-animation: '.$unique_class.' '.$anim_time.'s linear infinite; animation: '.$unique_class.' '.$anim_time.'s linear infinite; }' : null;
	!empty($anim_bg_keyframes) ? $js_styles .= $anim_bg_keyframes : null;
}
!empty($typography_string) || $font_color !== '' ? $js_styles .= $unique_class_css.' .wtbx_row_content_inner {' . wtbx_vc_color_styles_text($font_color) . $typography_string . '}' : null;
if ( $row_height_type === 'fixed_height_screen' ) {
	$js_styles .= $unique_class_css.' .wtbx_row_content_inner {' . $row_height . '}';
}
if ( !empty($row_min_height) ) {
	$js_styles .= $unique_class_css.' .wtbx_row_content_inner {' . $row_min_height . '}';
}
if ( $max_width !== '' ) {
	$js_styles .= $unique_class_css.'.wtbx_vc_row .wtbx_row_content {max-width:' . intval($max_width) . 'px; margin: 0 auto;}';
}


$margin_selector    = $unique_class_css . ' .' . $element_class . '_inner';
$border_selector    = $unique_class_css . ' .wtbx_row_bg';
$padding_selector   = $unique_class_css . ' .wtbx_row_content_inner';
$offset_selector    = $unique_class_css . ' .wtbx_row_content_wrapper';
...
$js_styles .= $color_border !== '' ? $unique_class_css.' .wtbx_row_bg {' . $color_border .'}' : '';
...
$output .= '<div class="wtbx_row_wrapper">';

if ( $allow_content ) {
	if ( !empty($bg_image_el) || !empty($bg_video_el) || !empty($bg_anim_el) || !empty($bg_overlay) || !empty($custom_bg_color) || !empty($border_color) ) {
		$output .= '<div class="wtbx_row_bg">';

		if ( !empty($bg_image_el) || !empty($bg_video_el) || !empty($bg_anim_el) || !empty($bg_overlay) || !empty($border_color) ) {
			$output .= '<div class="wtbx_row_bg_inner' . ($custom_bg_image !== '' ? ' wtbx-element-reveal' : '') . ' wtbx-reveal-cont'.$bg_parallax_cont_class.'">';
			$output .= '<div class="wtbx_row_bg_media_wrapper wtbx-entry-media'.$bg_scale_elem_class.'"'.$bg_scale.'>';
			$output .= '<div class="wtbx_row_bg_media'.$bg_parallax_elem_class.'"'.$bg_parallax.$bg_mousemove.'>';
			$output .= $bg_image_el;
			$output .= $bg_video_el;
			...
			if ( !empty($bg_anim_el) ) {
				$output .= '<div class="wtbx_row_bg_anim" data-disable="'. esc_attr(!empty($anim_disable) ? wtbx_vc_scape_screen_sizes()[$anim_disable] : '0') . '">';
				$output .= '<div class="wtbx_row_bg_anim_inner">';
				$output .= $bg_anim_el;
...
$output .= '<div class="wtbx_row_content clearfix">';
$output .= '<div class="wtbx_row_content_wrapper">';
$output .= '<div class="wtbx_row_content_inner' . (wtbx_vc_is_page_editable() ? ' wpb_row' : '') . ' clearfix">';
...
if ( wtbx_vc_is_page_editable() ) {
	$output .= "<script>";
	$output .= "if ('undefined' !== typeof SCAPE && 'undefined' !== typeof SCAPE.parallaxBackground && SCAPE.videoBg) {SCAPE.parallaxBackground.mousemove();SCAPE.videoBg.init();}";
	$output .= "jQuery('".$unique_class_css."').parent().css('z-index', jQuery('".$unique_class_css."').css('z-index'));";
	$output .= "if ( jQuery('".$unique_class_css."').hasClass('wtbx_force_align_top') ) {jQuery('".$unique_class_css."').parent().addClass('wtbx_force_align_top')};";
	$output .= "if ( jQuery('".$unique_class_css."').hasClass('wtbx_force_align_middle') ) {jQuery('".$unique_class_css."').parent().addClass('wtbx_force_align_middle')};";
	$output .= "jQuery('".$unique_class_css."').parent().css('z-index', jQuery('".$unique_class_css."').css('z-index'));";
	$output .= "</script>";
}
