...
add_action('admin_init', 'cmb2_conditionals_load_actions', 9999);
...
function cmb2_conditionals_load_actions()
{
	...
	add_action('cmb2_group_wrap_attributes', 'cmb2_conditionals_group_attributes', CMB2_CONDITIONALS_PRIORITY, 3);
}
...
//	wp_enqueue_script('cmb2-conditionals', plugins_url('/cmb2-conditionals.js', __FILE__ ), array('jquery'), '1.0.2', true);
	wp_enqueue_script( 'cmb2-conditionals', WTBX_PLUGIN_URL . '/include/metaboxes/js/metaboxes-conditionals.js', array( 'jquery' ), '', true );
}

//function cmb2_conditionals_group_attributes( $attr_string, $group_args ) {
...
function cmb2_conditionals_group_attributes($val,  $group_args ) {
	if ( !isset($group_args->args['attributes']['data-conditional-id']) || !isset($group_args->args['attributes']['data-conditional-value']) ) {
		return $val;
	}

	$val['data-conditional-id'] = $group_args->args['attributes']['data-conditional-id'];
	$val['data-conditional-value'] = $group_args->args['attributes']['data-conditional-value'];
	return $val;
}
