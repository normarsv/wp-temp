...
(function( $ ) {
	...
	redux.field_objects.wtbx_details = redux.field_objects.wtbx_details || {};

	var scroll = '';

	redux.field_objects.wtbx_details.init = function( selector ) {

		if ( !selector ) {
			selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-wtbx_details:visible' );
		}
		...
		$( selector ).each(
			function() {
				...
				el.find( ".redux-wtbx_details" ).sortable(
					{
						handle: ".drag",
						...
						out: function( event, ui ) {
							...
							redux.field_objects.wtbx_details.scrolling( $( this ).parents( '.redux-field-container:first' ) );
						},
				...
				//el.find( '.checkbox_wtbx_details' ).on('click', function() {
				//		if ( $( this ).is( ":checked" ) ) {
				...
				el.find( '.wtbx_details-add' ).on('click',
					function() {
	...
	redux.field_objects.wtbx_details.scrolling = function( selector ) {
		if (selector === undefined) {
			return;
		}

		var $scrollable = selector.find( ".redux-wtbx_details" );

		if ( scroll == 'up' ) {
			$scrollable.scrollTop( $scrollable.scrollTop() - 20 );
			setTimeout( redux.field_objects.wtbx_details.scrolling, 50 );
		} else if ( scroll == 'down' ) {
			$scrollable.scrollTop( $scrollable.scrollTop() + 20 );
			setTimeout( redux.field_objects.wtbx_details.scrolling, 50 );
		}
