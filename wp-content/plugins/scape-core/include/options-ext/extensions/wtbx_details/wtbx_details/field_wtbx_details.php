...
// Don't duplicate me!
if( !class_exists( 'ReduxFramework_wtbx_details' ) ) {
     ...
     * @since       1.0.0
     */
    class ReduxFramework_wtbx_details extends ReduxFramework {
...
//                $this->extension_url = site_url( str_replace( trailingslashit( str_replace( '\\', '/', ABSPATH ) ), '', $this->extension_dir ) );
        ...
        public function render() {
	        ...
	        echo '<ul id="' . $this->field['id'] . '-list" class="redux-wtbx_details wtbx-text-sortable ' . $class . '">';

	        $name  = 'name="' . $this->field['name'] . $this->field['name_suffix'] . '" ';
	        echo '<li class="wtbx-text-sortable-item wtbx-text-sortable-copy">';
	        echo '<span class="compact drag wtbx-handle"><i class="el el-lines icon-large"></i></span>';
	        echo '<input class="' . $class . '" type="' . $this->field['mode'] . '" ' . $name . 'id="' . $this->field['id'].'" value="" />';
	        echo '<a href="javascript:void(0);" class="deletion wtbx-text-sortable-item-remove" data-confirmation="'.esc_html__('Are you sure you want to remove this color from palette?', 'core-extension').'">' . esc_html__( 'Remove', 'core-extension' ) . '</a>';
	        echo '</li>';
...
//	        $this->field['add_number'] = ( isset( $this->field['add_number'] ) && is_numeric( $this->field['add_number'] ) ) ? $this->field['add_number'] : 1;
	        echo '<div class="wtbx-text-sortable-footer"><a href="javascript:void(0);" class="button button-primary wtbx_details-add wtbx-text-sortable-add" data-add_number="' . ($add_number + 1) . '" data-id="' . $this->field['id'] . '-list">' . $this->add_text . '</a></div>';
        ...
        public function enqueue() {
            ...
            $extension = ReduxFramework_extension_wtbx_details::getInstance();

            wp_enqueue_script(
                'redux-field-wtbx-details-js',
                $this->extension_url . 'field_wtbx_details.js',
                array( 'jquery' ),
                time(),
                true
            );
            ...
            wp_enqueue_style(
                'redux-field-wtbx-details-css',
                $this->extension_url . 'field_wtbx_details.css',
                time(),
                true
            );
