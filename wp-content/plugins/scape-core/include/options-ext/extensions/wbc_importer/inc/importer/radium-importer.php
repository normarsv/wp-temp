...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
		...
		public function __construct($options) {
			...
			foreach( $this->options as $type => $status ) {
				if ( in_array($type, array('theme-options', 'demo-content', 'widgets')) && $status === 'true' ) {
					$this->complete[$type] = 'pending';
				}
			}

			add_action( 'wbc_importer_after_content_import', array($this, 'demo_content_complete') );
			add_action( 'wbc_importer_after_theme_options_import', array($this, 'theme_options_complete') );
			add_action( 'radium_theme_import_widget_after_import', array($this, 'widgets_complete') );

			if ( isset($this->options['generate-resized-image-copies']) && $this->options['generate-resized-image-copies'] !== 'true' ) {
		...
		public function demo_content_complete() {
			$this->complete['demo-content'] = true;
			$this->maybe_finish_import();
		}

		public function theme_options_complete() {
			$this->complete['theme-options'] = true;
			$this->maybe_finish_import();
		}

		public function widgets_complete() {
			$this->complete['widgets'] = true;
			$this->maybe_finish_import();
		}

		public function maybe_finish_import() {
			foreach( $this->complete as $type => $status ) {
				if ( $status !== true ) {
					return;
				}
			}
		...
		public function process_imports() {
			...
			if ( isset($this->options['demo-content']) && $this->options['demo-content'] === 'true' ) {
				if ( !empty( $this->content_demo ) && is_file( $this->content_demo ) ) {
					$this->set_demo_data( $this->content_demo, $this->options );
				}
			} else {
				do_action( 'import_end' );
			}

			if ( isset($this->options['theme-options']) && $this->options['theme-options'] === 'true' ) {
			...
			if ( isset($this->options['widgets']) && $this->options['widgets'] === 'true' ) {
			...
			if( $registred_menus ) {
				foreach( $registred_menus as $menu ) {
					if( $menu->name === 'Main' || $menu->name === 'Main Navigation' ) {
		...
		public function set_demo_data( $file, $options ) {
			...
			if ( $importer_error ) {
				...
				if ( !is_file( $file ) ) {
					...
					if ( isset($this->options['attachments']) && $this->options['attachments'] === 'true' ) {
		...
		public function set_demo_theme_options( $file ) {
			...
			if ( ! file_exists( $file ) ) {
				wp_die(
					esc_html__( 'Theme options Import file could not be found. Please try again.', 'core-extension' ),
					'',
		...
		function process_widget_import_file( $file ) {
			...
			if ( ! file_exists( $file ) ) {
				wp_die(
					esc_html__( 'Widget Import file could not be found. Please try again.', 'core-extension' ),
					'',
					array( 'back_link' => true )
				);
			}
		...
		public function default_sidebars() {
			$array = array(
				'wtbx_sidebar_1122828712851770' => array(
					'name' => 'Blog sidebar',
					'id' => 'wtbx_sidebar_1122828712851770'
				),
				'wtbx_sidebar_2514202025784580' => array(
					'name' => 'Blog sidebar large',
					'id' => 'wtbx_sidebar_2514202025784580'
				),
				'wtbx_sidebar_7111081110217552' => array(
					'name' => 'Post sidebar',
					'id' => 'wtbx_sidebar_7111081110217552'
				),
				'wtbx_sidebar_5511169628211125' => array(
					'name' => 'Page sidebar',
					'id' => 'wtbx_sidebar_5511169628211125'
				),
				'wtbx_sidebar_4325010122831155' => array(
					'name' => 'Shop sidebar',
					'id' => 'wtbx_sidebar_4325010122831155'
				),
				'wtbx_sidebar_0100108080860785' => array(
					'name' => 'Product sidebar',
					'id' => 'wtbx_sidebar_0100108080860785'
				),
				'wtbx_sidebar_4881266410824211' => array(
					'name' => 'Latest products',
					'id' => 'wtbx_sidebar_4881266410824211'
				),
				'wtbx_sidebar_1806900909081105' => array(
					'name' => 'On sale products',
					'id' => 'wtbx_sidebar_1806900909081105'
				),
				'wtbx_sidebar_5565731578511783' => array(
					'name' => 'Portfolio sidebar',
					'id' => 'wtbx_sidebar_5565731578511783'
				),
				'wtbx_sidebar_4882117125271881' => array(
					'name' => 'Sidearea',
					'id' => 'wtbx_sidebar_4882117125271881'
				),
				'wtbx_sidebar_2157231251217223' => array(
					'name' => 'Search panel',
					'id' => 'wtbx_sidebar_2157231251217223'
				),
				'wtbx_sidebar_8111119191814611' => array(
					'name' => 'Side nav',
					'id' => 'wtbx_sidebar_8111119191814611'
				),
			);
...
//      global $wp_registered_sidebars;
			...
			$registered_sidebars = $this->default_sidebars();
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				if ( isset( $registered_sidebars[$sidebar_id] ) ) {
					...
					$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'core-extension' );
				}

				// Result for sidebar
				$results[$sidebar_id]['name'] = ! empty( $registered_sidebars[$sidebar_id]['name'] ) ? $registered_sidebars[$sidebar_id]['name'] : $sidebar_id; // sidebar name if theme supports it; otherwise ID
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
						...
						$widget_message = esc_html__( 'Site does not support widget', 'core-extension' ); // explain why widget not imported
					}
					...
					if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
						...
						foreach ( $single_widget_instances as $check_id => $check_widget ) {
							...
							if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
								...
								$widget_message = esc_html__( 'Widget already exists', 'core-extension' ); // explain why widget not imported
					...
					if ( ! $fail ) {
						...
						if ( $sidebar_available ) {
							...
							$widget_message = esc_html__( 'Imported', 'core-extension' );
						} else {
							$widget_message_type = 'warning';
							$widget_message = esc_html__( 'Imported to Inactive', 'core-extension' );
						}
					...
					$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'core-extension' ); // show "No Title" if widget instance is untitled
