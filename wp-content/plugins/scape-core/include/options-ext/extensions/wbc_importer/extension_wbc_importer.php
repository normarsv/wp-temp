...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function demoPlugins($demo) {
        	$return = '';
        	$array = array(
        		'agency-one' => ['Scape Core', 'WPBakery Page Builder', 'Contact Form 7', 'MailChimp'],
		        'blog-adventure' => ['Scape Core', 'WPBakery Page Builder'],
		        'blog-lifestyle' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'coming-soon-one' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'coming-soon-two' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'corporate-one' => ['Scape Core', 'WPBakery Page Builder'],
		        'corporate-two' => ['Scape Core', 'WPBakery Page Builder'],
		        'ico' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'interior' => ['Scape Core', 'WPBakery Page Builder'],
		        'landing-one' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'landing-two' => ['Scape Core', 'WPBakery Page Builder', 'Contact Form 7'],
		        'main' => ['Scape Core', 'WPBakery Page Builder', 'Contact Form 7', 'MailChimp', 'WooCommerce', 'YITH WooCommerce Wishlist'],
		        'minimal' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'mobile-app' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'portfolio-minimal' => ['Scape Core', 'WPBakery Page Builder'],
		        'portfolio-photography' => ['Scape Core', 'WPBakery Page Builder'],
		        'portfolio-vivid' => ['Scape Core', 'WPBakery Page Builder', 'Contact Form 7'],
		        'product-one' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'saas' => ['Scape Core', 'WPBakery Page Builder'],
		        'shop' => ['Scape Core', 'WPBakery Page Builder', 'MailChimp'],
		        'web-app' => ['Scape Core', 'WPBakery Page Builder']
	        );

        	if ( array_key_exists($demo, $array) ) {
	    ...
	    public function demoContent($demo) {
		    $return = json_encode(['Theme options', 'Demo content', 'Attachments', 'Generate resized image copies']);
		    $array = array(
			    'main' => ['Theme options', 'Demo content', 'Widgets', 'Attachments', 'Generate resized image copies']
		    );
            ...
            if ( !empty( $imports ) && is_array( $imports ) ) {
                ...
                foreach ( $imports as $import ) {
                    ...
                    if ( $import['type'] == "d" && !empty( $import['name'] ) ) {
	                    ...
	                    $this->wbc_import_files['wbc-import-'.$x]['plugins'] = $this->demoPlugins($import['name']);
	                    $this->wbc_import_files['wbc-import-'.$x]['options'] = $this->demoContent($import['name']);
        ...
        public function ajax_importer() {
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
	            ...
	            if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
		            ...
		            $installer = new Radium_Theme_Demo_Data_Importer( $this, $this->parent, $_REQUEST['options'] );
                }else {
                    echo esc_html__( "Demo Already Imported", 'core-extension' );
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'core-extension' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'core-extension' );

	        if ( SCAPE_Core_Extend::is_theme_activated() ) {
		        $this->parent->sections[] = array(
			        'id'     => 'wbc_importer_section',
			        'title'  => $wbc_importer_label,
			        'desc'   => '<p class="description importer-description"><strong>'. apply_filters( 'wbc_importer_description', esc_html__( 'In case of any problems with demo import, please refer to ', 'core-extension' ) ) . sprintf('<a href="http://docs.whiteboxstud.io/scape/#2-import" target="_blank">%s</a>', esc_html__('theme documentation', 'core-extension')) . '</strong></p>',
			        'icon'   => 'scape-ui-download',
			        'fields' => array(
				        array(
