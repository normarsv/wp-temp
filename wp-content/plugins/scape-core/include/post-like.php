...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'core-extension' ) );
	}
 ...
 * @since    0.5
 */
function get_simple_likes_button( $post_id, $filled = false, $is_comment = NULL ) {

	wtbx_vc_localize_main_js('simpleLikes', array(
		'ajaxurl'   => admin_url( 'admin-ajax.php' ),
		'likes_single'  => esc_html__( 'Like', 'core-extension' ),
		'likes_plural'  => esc_html__( 'Likes', 'core-extension' ),
		'like'      => esc_html__( 'Like', 'core-extension' ),
		'unlike'    => esc_html__( 'Unlike', 'core-extension' )
	));
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'core-extension' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'core-extension' );
		$icon = $icon_empty;
	}
	...
	if ( $filled ) {
	    $icon = $icon_full;
		$decoration .= '<div class="wtbx-like-anim">';
	    for ( $i=0; $i<10; $i++ ) {
		    $decoration .= '<span></span>';
        }
		$decoration .= '</div>';
        $label = '<div class="wtbx-like-label wtbx-text"><span class="wtbx-like-label-prefix">' . $title . '</span>' . esc_html__(' this ', 'core-extension') . strtolower(get_post_type_object( get_post_type($post_id) )->labels->singular_name) . '</div>';
	}
	$output = '<a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&nonce=' . $nonce . '&post_id=' . $post_id . '&disabled=true&is_comment=' . $is_comment ) . '" class="wtbx-like-button sl-button' . $post_id_class . $class . $comment_class . ($filled ? ' filled' : '') . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . '<span class="sl-count like-count">' . $count . '</span>' . $decoration . '</a>' . $label;
	return $output;
...
function get_liked_icon() {
	$icon = '<i class="scape-ui-heart-filled"></i>';
	return $icon;
} // get_liked_icon()
...
function get_like_count( $like_count ) {
	$like_text      = esc_html__( 'Like', 'core-extension' );
	$likes_single   = esc_html__( 'Like', 'core-extension' );
	$likes_plural   = esc_html__( 'Likes', 'core-extension' );
//	if ( is_numeric( $like_count ) && $like_count > 0 ) {
//		$number = sl_format_count( $like_count );
//		$number = $number == 1 ? $number . ' ' . $likes_single : $number . ' ' . $likes_plural;
//	} else {
//		$number = $like_text;
//	}
//	$count = $number;
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'core-extension' ); ?></label></th>
			<td>
				<?php
				...
				<?php else : ?>
					<p><?php _e( 'You do not like anything yet.', 'core-extension' ); ?></p>
				<?php
				endif;
				wp_reset_postdata();
				?>
